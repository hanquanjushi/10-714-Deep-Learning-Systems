...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        # sum up partial adjoint
        node.grad = sum(node_to_output_grads_list[node])
        # propagate partial adjoint to its input
        if node.op is None:
            continue
        partial_grad = node.op.gradient_as_tuple(node.grad, node)
        for prev, grad in zip(node.inputs, partial_grad):
            node_to_output_grads_list.setdefault(prev, []).append(grad)
   ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    """
    ### BEGIN YOUR SOLUTION
    # Create an empty list to store the topological sort order
    topo_order = []

    # Create a set to keep track of visited nodes
    visited = set()

    # Perform a post-order DFS traversal on each node in the input list
