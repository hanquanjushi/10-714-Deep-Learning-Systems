...
def softmax_loss(Z, y_one_hot):
    ...
    """
    ### BEGIN YOUR SOLUTION
    #return (ndl.ops.summation(ndl.ops.log(ndl.ops.summation(ndl.ops.exp(Z), axes = 1)))- ndl.ops.summation(Z*y_one_hot))/Z.shape[0]
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(step):
        idx = i*batch
        endidx = min(idx + batch, n) 
        X_batch =ndl.Tensor( X[idx:endidx])
        y_batch = y[idx:endidx]
